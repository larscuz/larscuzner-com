...
import { getPublishedPages } from "@/lib/server/public-site";
        ...
        <header className="border-b border-white/10 pb-6">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-5 text-xs uppercase tracking-[0.3em] text-white/35">
              ...
              <Link href="/info" className="text-white/70">
                Info
              </Link>
          ...
          <p className="mt-4 max-w-2xl text-sm leading-7 text-white/58">
            Published pages carried over into the new composer-driven site. Commerce utility pages are hidden from this
            public overview for now.
          </p>
        </header>

        <section className="grid gap-px py-8 md:grid-cols-2">
          {pages.map((entry) => (
            <Link
              key={entry.id}
            ...
            >
              <p className="text-[0.72rem] uppercase tracking-[0.34em] text-white/28">{entry.kind}</p>
              <h2 className="mt-6 text-3xl font-semibold tracking-[-0.05em] text-white transition group-hover:text-white/92">
                {entry.title}
              </h2>
              <p className="mt-6 max-w-md text-sm leading-7 text-white/55">{entry.excerpt || "Open page"}</p>
